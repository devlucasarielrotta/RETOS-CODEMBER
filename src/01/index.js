...
const totalPalabras = texto.split(' ');
const totalPalabrasMinusculas = totalPalabras.map(palabra =>  palabra.toLocaleLowerCase());

const palabrasOBJ = {};

totalPalabrasMinusculas.forEach(palabra => {
  if (!palabrasOBJ[palabra]) {
    palabrasOBJ[palabra]= {
        repeticiones : 1
    }
   
  } 
  palabrasOBJ[palabra]['repeticiones'] ++;
});

let mensaje ='';
for (const iterator in palabrasOBJ) {
    mensaje+= iterator + palabrasOBJ[iterator].repeticiones
}
